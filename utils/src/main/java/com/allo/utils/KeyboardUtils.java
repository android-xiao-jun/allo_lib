...
//                + (decorView.getBottom() - outRect.bottom));
        ...
        if (delta <= StatusBarUtils.INSTANCE.getNavBarHeight() + StatusBarUtils.INSTANCE.getStatusBarHeight()) {
    ...
    private static int getContentViewInvisibleHeight(final Window window) {
        ...
        if (delta <= StatusBarUtils.INSTANCE.getStatusBarHeight() + StatusBarUtils.INSTANCE.getNavBarHeight()) {
            return 0;
        }
        return delta;
    }
